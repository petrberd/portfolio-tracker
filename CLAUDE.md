...
## Stack
Next.js 14 (App Router) · TypeScript · Recharts · SheetJS (xlsx) · Tailwind · dnd-kit. Port 3210.
...
- UI: `app/page.tsx` (dashboard) + `components/` (Charts, Analysts, StockDetail, DividendCalendar,
  ...
  SectionOrder — React kontexty pro skrývání/přesouvání sekcí; SortableBlock — dnd-kit wrapper
  s úchytem; IconButton — malé ikonové tlačítko s hover tooltipem, použité pro „skrýt sekci").
...
2. **Atribuce výnosu** — top přispěvatelé / detraktoři.
3. **Přepínač CZK/USD** zobrazení (historický FX už je opravený, tohle by byl další krok).
4. **Portabilita pro jiného uživatele** (audit proveden, neopraveno — nízká priorita, dokud appku
...
(Headline strip s celkovým výnosem a rizikovým flagem — implementováno v v1.4.0. Sledované
tituly/wishlist a skrývání/přesouvání sekcí — implementováno v v1.7.0.)
...
- **Finnhub odstraněn, appka teď nepotřebuje žádný API klíč** — sektor jede přes `lib/sector.ts`
  ...
  smazány.
- **Wishlist + skrývání/přesouvání sekcí** (v1.7.0) — tři nové, na sobě nezávislé server-side
  perzistované preference (`lib/wishlist.ts`, `lib/sectionVisibility.ts`, `lib/sectionOrder.ts`),
  všechny se stejnou serializovanou read-modify-write ochranou (viz Architektura výše — bez ní
  dva rychlé zápisy za sebou jeden druhého přepisovaly, reálně narazeno a opraveno). Přesouvání
  sekcí používá `dnd-kit` (`components/SortableBlock.tsx`); Alokace+Pozice+Earnings a
  Dividendy+Vklady se táhnou jako jeden blok, ne po jednotlivých sekcích, aby zůstalo zachované
  side-by-side rozložení na desktopu. Skutečný drag gesto se nepodařilo ověřit automatizovaně
  v tomto sandboxu (syntetické pointer/keyboard eventy se nedostaly přes dnd-kit aktivační
  logiku) — ověřeno ručně uživatelem v prohlížeči, funguje.
