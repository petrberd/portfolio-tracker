...
import { alertTriggered, createWishlistStore } from "@/lib/wishlist";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";

// Own store (data/demoWishlist.json) — shared across all demo visitors, but never the
// real portfolio's file. See app/api/demo/holding-alerts for the same pattern.
const demoWishlist = createWishlistStore("demoWishlist.json");

async function enriched(force = false) {
  const items = await demoWishlist.loadWishlist();
...
export async function POST(req: NextRequest) {
  ...
  await demoWishlist.addWishlistItem(symbol, typeof name === "string" && name ? name : symbol);
...
export async function DELETE(req: NextRequest) {
  ...
  await demoWishlist.removeWishlistItem(symbol);
  return NextResponse.json({ items: await enriched() });
}
...
export async function PATCH(req: NextRequest) {
  ...
  if (clear) {
    await demoWishlist.setWishlistAlert(symbol, null);
  } else {
    ...
    await demoWishlist.setWishlistAlert(symbol, { targetPrice: price, direction });
  }
  return NextResponse.json({ items: await enriched() });
}
